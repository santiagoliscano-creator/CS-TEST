"""
Hotmart Club · Club Analytics v5.1
...
/* Alerts */
.stSuccess { background: #f0faf5 !important; border-left-color: #1aab6d !important; border-radius: 10px !important; }
.stError   { background: #fff5f2 !important; border-left-color: #E8420A !important; border-radius: 10px !important; }
.stWarning { background: #fffbf0 !important; border-radius: 10px !important; }
.stInfo    { background: #f5f0ff !important; border-radius: 10px !important; }
...
for key, default in [
    ("page", "login"), ("token", None), ("modulo_info", {}),
    ("subdomain", ""), ("club_name", ""),
    ...
    with col_center:
        ...
        <div style="text-align:center; margin-bottom:28px;">
            ...
            <h1 style="font-family:'Nunito Sans',sans-serif; font-weight:800; font-size:28px;
                       color:#1a1815; margin:0 0 6px;">Club Analytics</h1>
            <p style="font-family:'Nunito Sans',sans-serif; color:#8c8880; font-size:14px; margin:0;">
        ...
        st.markdown("""
        <div style="background:white; border-radius:20px; padding:32px 32px 8px;
            ...
            <p style="font-family:'Nunito Sans',sans-serif; color:#8c8880; font-size:13px;
                       margin:0 0 24px;">Ingresa tus credenciales de Hotmart Developers</p>
        </div>
        """, unsafe_allow_html=True)

        # Formulario — usando componentes nativos de Streamlit dentro del card visual
        with st.container():
            st.markdown("""
            <div style="background:white; border-radius:0 0 20px 20px; padding:0 32px 28px;
                        box-shadow:0 8px 40px rgba(0,0,0,0.08); border:1px solid #f0ede8;
            ...
            client_secret = st.text_input("Client Secret",       placeholder="Tu client secret", type="password", key="l_secret")
        ...
        <div style="text-align:center; margin-top:20px; padding-bottom:40px;">
            <p style="font-family:'Nunito Sans',sans-serif; color:#b8b4ae; font-size:12px; line-height:1.6;">
                ...
                with st.spinner("Verificando credenciales y cargando módulos..."):
                    ...
                    if err:
                        ...
                        if not modulo_info:
                            ...
                            st.session_state["subdomain"]   = subdomain_in
                            st.session_state["club_name"]   = subdomain_in.replace("-", " ").title()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="display:flex; align-items:center; gap:14px;
                padding:20px 0 24px; border-bottom:2px solid #f0ede8; margin-bottom:32px;">
        <div style="width:44px; height:44px; background:#E8420A; border-radius:12px;
                    display:flex; align-items:center; justify-content:center;
                    box-shadow:0 4px 12px rgba(232,66,10,0.3); font-size:22px;">🔥</div>
        <div>
            <div style="font-family:'Nunito Sans',sans-serif; font-weight:800;
                        font-size:20px; color:#1a1815;">Club Analytics</div>
    ...
    <h2 style="font-family:'Nunito Sans',sans-serif; font-weight:800; font-size:22px;
               color:#1a1815; margin-bottom:6px;">¿Qué producto quieres analizar?</h2>
    ...
    col_sel, col_preview = st.columns([1.4, 1])
    ...
    with col_sel:
        ...
        if seleccionados:
            pages_txt = f"<div style='font-size:14px;color:#8c8880;margin-top:2px;'>{total_lecciones} lecciones en total</div>" if total_lecciones > 0 else ""
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,#fff5f2,#fff); border:1.5px solid #ffd4c4;
                ...
                <div style="font-family:'Nunito Sans',sans-serif; font-size:11px; font-weight:700;
                            color:#E8420A; letter-spacing:0.08em; margin-bottom:4px;">RESUMEN</div>
                <div style="font-family:'Nunito Sans',sans-serif; font-size:26px; font-weight:800;
    ...
    with col_preview:
        st.markdown("""
        <div style="font-family:'Nunito Sans',sans-serif; font-weight:700; font-size:11px;
                    color:#8c8880; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:10px;">
            MÓDULOS DISPONIBLES
        </div>
        """, unsafe_allow_html=True)
        for nombre, info in modulo_info.items():
            ...
            <div style="background:{bg}; border:1.5px solid {border}; border-radius:10px;
                ...
                <div>
                    ...
                    <div style="font-family:'Nunito Sans',sans-serif; font-size:11px; color:#b8b4ae;">
                        Principal{extra}{pages_txt}
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    col_back, _, col_go = st.columns([1, 2, 1])
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_c:
        ...
        <div style="text-align:center; margin-bottom:32px;">
            ...
            <h2 style="font-family:'Nunito Sans',sans-serif; font-weight:800; font-size:24px;
                       color:#1a1815; margin-bottom:8px;">Analizando tu Club...</h2>
            <p style="font-family:'Nunito Sans',sans-serif; color:#8c8880; font-size:14px; margin:0;">
                Extrayendo el progreso de cada alumno.<br>Esto puede tomar unos segundos.
            </p>
        </div>
        """, unsafe_allow_html=True)
        status_txt = st.empty()
    ...
    for i, student in enumerate(students):
        ...
        <p style="font-family:'Nunito Sans',sans-serif; text-align:center;
                  color:#8c8880; font-size:13px; margin-top:8px;">
        ...
        if not lecciones:
            all_data.append({
                ...
                "Completada": "No", "Fecha Completado": "", "Pct Hotmart": pct_hotmart
            })
            time.sleep(0.15)
            continue

        for l in lecciones:
            mod_nombre = l.get("module_name", "Sin modulo")
            if mod_nombre not in modulos_sel:
                continue
            fecha = ""
    ...
    status_txt.markdown("""
    <p style="font-family:'Nunito Sans',sans-serif; text-align:center;
              color:#1aab6d; font-size:14px; font-weight:700; margin-top:8px;">✓ ¡Listo!</p>
    """, unsafe_allow_html=True)
    ...
    for nombre, grupo in df.groupby("Nombre"):
        ...
        if grupo_activo.empty:
            pct = min(float(pct_hotmart) if pct_hotmart else 0.0, 100.0)
            resumen_rows.append({
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_h1:
        ...
        <div style="padding:20px 0 20px;">
            <div style="display:flex; align-items:center; gap:12px; margin-bottom:6px;">
                <div style="width:40px;height:40px;background:#E8420A;border-radius:11px;
                            display:flex;align-items:center;justify-content:center;
                            box-shadow:0 3px 10px rgba(232,66,10,0.3);font-size:20px;">🔥</div>
                <span style="font-family:'Nunito Sans',sans-serif;font-weight:800;
                             font-size:22px;color:#1a1815;">Club Analytics</span>
                <span style="background:#fff5f2;color:#E8420A;border:1px solid #ffd4c4;
                             border-radius:20px;padding:3px 12px;font-size:12px;
                             font-weight:700;font-family:'Nunito Sans',sans-serif;">{club_name}</span>
            </div>
            <p style="font-family:'Nunito Sans',sans-serif;color:#8c8880;font-size:13px;margin:0;">
                {len(modulos_sel)} módulos · Generado el {datetime.now().strftime('%d/%m/%Y a las %H:%M')}
            </p>
        </div>
        """, unsafe_allow_html=True)
    with col_h2:
    ...
    st.markdown("""
    <div style="height:3px; background:linear-gradient(90deg,#E8420A,#ff9a7a,transparent);
    ...
    if sin_actividad > 0:
        ...
        <div style="background:#fff5f2;border:1.5px solid #ffd4c4;border-radius:12px;
            ...
            <span style="font-family:'Nunito Sans',sans-serif;font-size:14px;
                         color:#c93608;font-weight:600;">
                {sin_actividad} alumno{'s' if sin_actividad > 1 else ''} nunca
                {'accedieron' if sin_actividad > 1 else 'accedió'} al Club —
    ...
    with tab1:
        ...
        with col_l:
            ...
            fig_d.update_layout(margin=dict(t=20,b=20,l=20,r=20), height=300, **plot_defaults)
        ...
        with col_r:
            ...
            fig_a = go.Figure(go.Bar(
                ...
                hovertemplate="<b>%{y}</b><br>%{x}% (%{customdata[0]}/%{customdata[1]})<br>Hotmart oficial: %{customdata[2]}%<extra></extra>"
            ))
            fig_a.update_layout(
                xaxis=dict(range=[0,115], ticksuffix="%", showgrid=True, gridcolor="#f0ede8", zeroline=False),
    ...
    with tab2:
        ...
        with col_a:
            ...
            if sin_act.empty:
                ...
                st.error(f"{len(sin_act)} alumnos sin lecciones completadas:")
                st.dataframe(sin_act, use_container_width=True, hide_index=True)
        with col_b:
            st.markdown("**Módulos con más pendientes**")
            if not abandono_mod.empty:
                ...
                fig_c.update_layout(
                    xaxis_title="Lecciones pendientes",
                    yaxis=dict(autorange="reversed"),
                    margin=dict(t=10,b=10,l=10,r=50), height=280, **plot_defaults
                )
    ...
    with tab3:
        if not df_pivot.empty:
            ...
            st.markdown("---")
            st.markdown("**Zoom: avance por alumno en un módulo**")
    ...
    with tab4:
        st.markdown("**Lecciones que cada alumno NO ha completado todavía**")
    ...
    with tab5:
        ...
        st.caption("Más oscuro = más avance")
    ...
    st.markdown("""
    <div style="height:3px; background:linear-gradient(90deg,#E8420A,#ff9a7a,transparent);
                border-radius:2px; margin:24px 0 20px;"></div>
    """, unsafe_allow_html=True)
    ...
    with col_txt:
        ...
        <p style="font-family:'Nunito Sans',sans-serif; font-weight:700; font-size:15px;
                   color:#1a1815; margin-bottom:4px;">Exportar informe completo</p>
        <p style="font-family:'Nunito Sans',sans-serif; color:#8c8880; font-size:13px; margin:0;">
            Excel con 5 pestañas: resumen, por módulo, tabla cruzada, pendientes y detalle.
        </p>
        """, unsafe_allow_html=True)
    with col_btn:
        buffer = io.BytesIO()
